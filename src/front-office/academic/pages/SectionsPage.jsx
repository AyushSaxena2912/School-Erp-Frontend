...
export default function SectionsPage() {
  const { sections, addSection, updateSection, deleteSection } = useAcademic();
  const list = usePagedList(sections, (r) => r.name);
  ...
  const save = (e) => {
    ...
    if (editing) {
      updateSection({ id: editing.id, name: name.trim(), status });
    } else {
      addSection({ name: name.trim(), status });
    }
    setModalOpen(false);
  };
  ...
  const handleExport = () => {
    ...
    const rows = sections.map((r) =>
      [r.name, r.status].map((v) => `"${String(v).replace(/"/g, '""')}"`).join(",")
    );
  ...
  const handleImportFile = (e) => {
    ...
    reader.onload = (ev) => {
      ...
      const existingNames = new Set(sections.map((s) => s.name.trim().toLowerCase()));
      let imported = 0, skipped = 0;
      const errors = [];
      lines.slice(1).forEach((line, idx) => {
        const cols = line.replace(/^"|"|"$/g, "").split(/","/).map((v) => v.trim());
        const [sName, sStatus = "Active"] = cols;
        if (!sName) { errors.push(`Row ${idx + 2}: Name is empty.`); skipped++; return; }
        if (existingNames.has(sName.toLowerCase())) { skipped++; return; }
        addSection({ name: sName, status: ["Active", "Inactive"].includes(sStatus) ? sStatus : "Active" });
  ...
  const handleDeleteSelected = () => {
    ...
    if (
      window.confirm(
        `Delete ${count} selected section${count > 1 ? "s" : ""}?`
      )
    ) {
      list.selected.forEach((id) => deleteSection(id));
      list.setSelected([]);
    }
  };
      ...
      <AcademicListShell
        ...
        primaryAction={
          <div className="flex flex-wrap items-center gap-2">
            <input ref={importRef} type="file" accept=".csv" className="hidden" id="sections-import" onChange={handleImportFile} />
            <button type="button" className={btnSecondary} onClick={() => importRef.current?.click()} title="Import sections from CSV">
              <svg className="h-[15px] w-[15px]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                <path strokeLinecap="round" strokeLinejoin="round" d="M4 16v2a2 2 0 002 2h12a2 2 0 002-2v-2M7 10l5 5 5-5M12 15V3" />
              </svg>
              Import
            </button>
            <button type="button" className={btnSecondary} onClick={handleExport} title="Export sections to CSV">
              <svg className="h-[15px] w-[15px]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                    ...
                    <RowMenu
                      items={[
                        ...
                        },
                        {
                          ...
                          onClick: () => {
                            if (
                              window.confirm(`Delete section "${row.name}"?`)
                            ) {
